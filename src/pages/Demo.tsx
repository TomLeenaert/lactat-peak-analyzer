...
const StepAthlete = ({ active }: { active: boolean }) => {
  ...
  const field = (label: string, value: React.ReactNode, show = true) => (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {proto.map((p, i) => (
            <div key={p.label} style={{
              ...
              <span style={{ fontSize: '13px', color: 'rgba(255,255,255,0.55)' }}>{p.icon} {p.label}</span>
...
const Demo = () => {
  ...
  const panels = [
    ...
    <StepProtocol active={step === 1} />,
    <StepData active={step === 2} />,
    <StepResults active={step === 3} />,
  ];
