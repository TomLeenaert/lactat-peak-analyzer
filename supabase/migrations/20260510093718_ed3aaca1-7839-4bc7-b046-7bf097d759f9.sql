-- Allow anonymous visitors to log page_view events via log_event RPC
GRANT EXECUTE ON FUNCTION public.log_event(text, jsonb) TO anon;

-- Allow anon insert into analytics_events but ONLY for visit events with no user_id
CREATE POLICY "Anon can insert visit events"
ON public.analytics_events
FOR INSERT
TO anon
WITH CHECK (user_id IS NULL AND event_type IN ('page_view', 'visit'));

-- Update admin_overview to include visitor stats
